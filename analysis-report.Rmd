...
The original data set contains many variables, including metadata, the sensor data, and statistical indicators computed a posteriori using temporal rolling windows on the sensor data.

Apart from this introductory section the report contains an Exploratory Analysis section in which the data is analysed and cleaned to only leave data that can be used in the predicition. The Predicive Model section present the construction of the model itself, including preprocessing (PCA) and the training of a Random Forest algorithm. The Random Forest model was selected because of the nature of the problem (categorical classification) and it's much better performance compared to other tree based algorithms. The last Results section presents the results of the prediction on the test data set.

Information about the HAR project that provided the data, the data set and a paper published with their work can be found [here](http://groupware.les.inf.puc-rio.br/har#weight_lifting_exercises)
...
Let's load the testing data. It's important that we do this now and not before the training phase, since this guarantees that we haven't has access to the testing data during the training phase:
...
We can see that column 160 is not the same on both data sets.
...
The column containing the outcome in the training data set has been replaced with the problem number for the project submission. We will limit ourselves to list our predictions here for each problem ID. Since only the outcome column has been replaced we can use the predictor selector we used for the training data unchanged. We will first compute the principal components for the test data set, *using the principal component object from the training data*, and then do the prediction:
...
This yields a probability of `r round(100 * prob.1in20, 1)` percent, which shows is prefectly fine that we got one answer wrong with our expected out of sample error.
